...
class PDFExtractorGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 다크 모드 토글 버튼
        self.dark_mode_btn = QPushButton("🌙 다크 모드")
        self.dark_mode_btn.setCheckable(True)
        self.dark_mode_btn.setChecked(self.dark_mode)
        self.dark_mode_btn.clicked.connect(self.toggle_dark_mode)
        top_button_layout.addWidget(self.dark_mode_btn)
    ...
    def create_main_tab(self):
        ...
        self.drop_label.filesDropped.connect(self.add_dropped_files)
    ...
    def toggle_dark_mode(self):
        ...
        if hasattr(self, 'dark_mode_btn'):
            if self.dark_mode:
                self.dark_mode_btn.setText("☀️ 라이트 모드")
            else:
                self.dark_mode_btn.setText("🌙 다크 모드")
            self.dark_mode_btn.setChecked(self.dark_mode)
    ...
    def update_stylesheets(self):
        ...
        if hasattr(self, 'file_progress_bar'):
            self.file_progress_bar.setStyleSheet(progress_style)
        ...
        try:
            ...
            if hasattr(self, 'cpu_label'):
                ...
                if cpu_percent > 80:
                    ...
                    theme = DARK_THEME if self.dark_mode else LIGHT_THEME
                    self.cpu_label.setStyleSheet(f"color: {theme['primary']}; font-weight: bold;")  # 초록색
            ...
            if hasattr(self, 'memory_label'):
                ...
                if memory_percent > 85:
                    ...
                    theme = DARK_THEME if self.dark_mode else LIGHT_THEME
                    self.memory_label.setStyleSheet(f"color: {theme['primary']}; font-weight: bold;")  # 초록색
...
class FileDropLabel(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        """)
        self.setText("📁 PDF 파일을 여기에 드래그하여 추가하세요\n\n또는 위의 버튼을 클릭하세요")
    ...
    def dropEvent(self, event):
        ...
        if files:
            ...
            self.setText(f"✅ {len(files)}개 파일이 추가되었습니다!")
    ...
    def add_dropped_files(self, files: list):
        """드래그 앤 드롭된 파일들 추가"""
        if files:
            # 기존 파일들과 합치기 (중복 제거)
            existing_files = set(self.pdf_files)
            ...
            # 성공 메시지
            if new_files:
                self.add_log_message(f"📁 {len(new_files)}개 파일이 드래그 앤 드롭으로 추가되었습니다.")
                # 드롭 라벨 텍스트 잠시 업데이트
                original_text = "📁 PDF 파일을 여기에 드래그하여 추가하세요\n\n또는 위의 버튼을 클릭하세요"
                self.drop_label.setText(f"✅ {len(new_files)}개 파일 추가됨!")
                
                # 3초 후 원래 텍스트로 복원
                QTimer.singleShot(3000, lambda: self.drop_label.setText(original_text))
            else:
                self.add_log_message("⚠️  선택된 파일들이 이미 목록에 있습니다.")
